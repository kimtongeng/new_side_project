...
1. Package the updated files into simplified category directories:
...
2. **Name the ZIP file using a descriptive task name** representing what was built/fixed (e.g. `Account_Location_Filtering_and_Mobile_Fixes.zip`). Never use generic names like `update.zip`.

3. Always save the output ZIP archive directly to:
   `C:\side project\new side\update_ai\`
