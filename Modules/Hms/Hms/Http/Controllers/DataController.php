 ...
 */
    ...
    public function CustomDashboardOptions()
    {
        return [
            ...
            [
                'name' => 'customdashboard_widget_hms_upcoming_booking',
                'label' => __('hms::lang.upcoming_bookings'),
                'size' => 100,
                'module_name' => __('hms::lang.hms_module'),
                'range' => false,
                'html_text' => false,
            ...
            [
                'name' => 'customdashboard_widget_hms_past_booking',
                'label' => __('hms::lang.past_bookings'),
                'size' => 100,
                'module_name' => __('hms::lang.hms_module'),
                'range' => false,
                'html_text' => false,
    ...
    public function customdashboard_widget_hms_upcoming_booking($dashboard_detail)
    {
        $business_id = session()->get('user.business_id');
        
        // Fixed window: next 30 days (including today)
        $window_start = Carbon::now()->startOfDay();
        $window_end = Carbon::now()->addDays(30)->endOfDay();

        $bookings_query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('contacts as c', 'transactions.contact_id', '=', 'c.id')
            ->where('transactions.type', 'hms_booking')
            ->whereBetween('transactions.hms_booking_arrival_date_time', [$window_start, $window_end])
            ->select(
                'transactions.id',
                'transactions.ref_no',
                'transactions.status',
        ...
        $bookings = $bookings_query
            ->orderBy('transactions.hms_booking_arrival_date_time', 'asc')
            ->limit(10)
            ->get();

        return [
            'html' => view('hms::custom_dashboard.upcoming_bookings', compact('bookings', 'window_start', 'window_end', 'dashboard_detail')),
        ];
    }

    public function customdashboard_widget_hms_past_booking($dashboard_detail)
    {
        $business_id = session()->get('user.business_id');

        // Fixed window: past 30 days (excluding today)
        $window_start = Carbon::now()->subDays(30)->startOfDay();
        $window_end = Carbon::now()->subDay()->endOfDay();

        $bookings_query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('contacts as c', 'transactions.contact_id', '=', 'c.id')
            ->where('transactions.type', 'hms_booking')
            ->whereBetween('transactions.hms_booking_departure_date_time', [$window_start, $window_end])
            ->select(
                'transactions.id',
                'transactions.ref_no',
                'transactions.status',
        ...
        $bookings = $bookings_query
            ->orderBy('transactions.hms_booking_departure_date_time', 'desc')
            ->limit(10)
            ->get();

        return [
            'html' => view('hms::custom_dashboard.past_bookings', compact('bookings', 'window_start', 'window_end', 'dashboard_detail')),
        ];
    }
}
