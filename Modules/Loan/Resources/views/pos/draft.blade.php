...
@section('title', __('Loan::lang.draft_sales'))

@section('content')
<!-- Content Header (Page header) -->
<section class="content-header no-print">
    <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">@lang('Loan::lang.draft_sales')</h1>
</section>
...
<section class="content no-print">
    ...
    @component('components.widget', ['class' => 'box-primary', 'title' => __('Loan::lang.draft_sales')])
        <div class="table-responsive">
            <table class="table table-bordered table-striped" id="draft_sales_table">
                <thead>
                    <tr>
                ...
                <tbody id="online_draft_sales">
                </tbody>
                <tbody id="offline_draft_sales" style="display: none;">
                </tbody>
            </table>
        </div>
    @endcomponent
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var draftSalesTable = $('#draft_sales_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ action([\Modules\Loan\Http\Controllers\LoanPosUtilityController::class, 'getRecentTransactions']) }}?status=draft',
                    data: function(d) {
                ...
                "fnDrawCallback": function(oSettings) {
                    __currency_convert_recursively($('#draft_sales_table'));
                }
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.is_online) {
                            ...
                            $('#offline_draft_sales').hide();
                            $('#online_draft_sales').show();
                            ...
                            $('#online_draft_sales').hide();
                            $('#offline_draft_sales').show();
                            loadOfflineDrafts();
                        }
                    },
                    error: function() {
                ...
                db.transactions.toArray().then(transactions => {
                    ...
                    transactions.forEach(transaction => {
                        ...
                        if (data.status === 'draft') {
                            let row = `
                                <tr class="offline_draft">
                                    <td>
                                        ...
                                        <button class="btn btn-xs btn-danger delete_offline_draft" data-offline_id="${transaction.offline_id}"><i class="fa fa-trash"></i> @lang("messages.delete")</button>
                                    </td>
                            ...
                            $('#offline_draft_sales').append(row);
                        }
                    });
                });
            }
            ...
            $(document).on('click', '.delete_offline_draft', function() {
