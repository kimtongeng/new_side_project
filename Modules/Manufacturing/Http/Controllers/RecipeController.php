...
class RecipeController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($recipes)
                ...
                ->addColumn('recipe_name', function ($row) {
                    $formatted_pname = \App\Utils\ProductUtil::getFormattedProductName($row->p_name, $row->secondary_name, true);
                    if ($row->product_type == 'variable') {
                        return $formatted_pname . ' - ' . $row->pv_name . ' - ' . $row->v_name . ' (' . $row->sub_sku . ')';
                    }
                    return $formatted_pname . ' (' . $row->sub_sku . ')';
                })
                ...
                ->rawColumns(['action', 'recipe_name', 'recipe_total', 'total_quantity', 'unit_cost', 'row_select'])
                ->make(true);
        }
    ...
    public function addIngredients()
    {
        ...
        $variation = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                            ...
                            ->select('p.name as product_name', 'p.secondary_name', 'p.type as product_type', 'variations.*', 'pv.name as product_variation_name', 'p.unit_id as unit_id', 'u.short_name as unit_name')
                            ->findOrFail($variation_id);
