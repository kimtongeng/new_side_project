...
$(document).ready(function () {
    ...
    $('#exchange_rate').change(function () {
        ...
        $('span#total_payable_exchange').each(function (index, ele) {
            let data = JSON.parse($(ele).attr('data-item'));
            $(ele).text(
                `${data.code}(${data.symbol}) ${__currency_trans_from_en(
                    shown_total * data.exchange_rate,
                    false
                )}`
            );
        });
